...
export default function FoundingPartnerForm({ ko }: { ko: boolean }) {
  ...
  return (
    <form id="partner-application" onSubmit={submit} className="scroll-mt-28 border border-green-deep/15 bg-white p-6 shadow-[0_24px_70px_rgba(23,76,58,.09)] sm:p-10">
      ...
      <label className="mt-7 flex cursor-pointer items-start gap-3 border-t border-green-deep/10 pt-6 text-xs leading-6 text-charcoal/60"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} required className="mt-1 size-4 accent-green-deep"/><span>{ko ? "가입 검토와 연락을 위해 위 개인정보와 신청 내용을 씨앗의 소리에 전달하는 데 동의합니다. 제출 내용은 창립파트너 협의 외의 목적으로 공개하지 않습니다." : "I agree to share this information with SEED VOICE for application review and contact. It will not be made public without prior agreement."}</span></label>
