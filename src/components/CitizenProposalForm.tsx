...
export default function CitizenProposalForm({ ko }: { ko: boolean }) {
  ...
  const mailto = `mailto:seedvoicekr@gmail.com?subject=${encodeURIComponent(`[${ko ? "시민제안" : "Citizen Proposal"}] ${draft.title}`)}&body=${encodeURIComponent(emailBody)}`;
  ...
  return (
    <>
      <form id="proposal-form" onSubmit={submit} className="scroll-mt-28 border border-green-deep/15 bg-white p-6 shadow-[0_20px_60px_rgba(23,76,58,.08)] sm:p-10">
        ...
        <label className="mt-7 flex cursor-pointer items-start gap-3 border-t border-green-deep/10 pt-6 text-xs leading-6 text-charcoal/60"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} required className="mt-1 size-4 accent-green-deep"/><span>{ko ? "제안 검토와 회신을 위해 이름·이메일과 제안 내용을 씨앗의 소리에 전달하는 데 동의합니다. 공개할 경우에는 사전에 제안자와 협의합니다." : "I agree to share my name, email and proposal with SEED VOICE for review and response. Publication will be discussed with the proposer in advance."}</span></label>
