  ...
  return <footer className="border-t border-white/10 bg-[#112B25] py-8 text-white">
    <div className="container-page">
      <div className="grid gap-7 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <div className="grid grid-cols-2 gap-6 border-t border-white/15 pt-5 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0"><div><p className="text-[10px] font-extrabold tracking-[.18em] text-white/35">READ</p><nav className="mt-3 grid gap-2 text-sm text-white/65"><Link to="/news">{ko ? "오늘의뉴스" : "Today's News"}</Link><Link to="/briefings">{ko ? "시민브리핑" : "Civic Briefings"}</Link><Link to="/monitoring">{ko ? "씨앗의 눈" : "SEED Watch"}</Link><Link to="/columns">{ko ? "씨앗의 소리" : "Voice of the Seed"}</Link></nav></div><div><p className="text-[10px] font-extrabold tracking-[.18em] text-white/35">ABOUT</p><nav className="mt-3 grid gap-2 text-sm text-white/65"><Link to="/about">{ko ? "씨드 보이스 소개" : "About SEED VOICE"}</Link><Link to="/publisher-message">{ko ? "발행인 인사말" : "Publisher's Message"}</Link></nav></div></div>
      </div>
