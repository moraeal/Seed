  ...
  return <footer className="border-t border-white/10 bg-[#112B25] py-8 text-white">
    <div className="container-page">
      <div className="grid gap-7 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <div className="grid grid-cols-2 gap-6 border-t border-white/15 pt-5 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0"><div><p className="text-[10px] font-extrabold tracking-[.18em] text-white/35">READ</p><nav className="mt-3 grid gap-2 text-sm text-white/65"><Link to="/columns">{ko ? "칼럼" : "Columns"}</Link><Link to="/news">{ko ? "핫이슈" : "Hot Issues"}</Link><Link to="/briefings">{ko ? "브리핑" : "Briefings"}</Link><Link to="/monitoring">{ko ? "시민감시" : "Civic Watch"}</Link><Link to="/seed-language">{ko ? "용어해설" : "Glossary"}</Link></nav></div><div><p className="text-[10px] font-extrabold tracking-[.18em] text-white/35">ABOUT</p><nav className="mt-3 grid gap-2 text-sm text-white/65"><Link to="/about">{ko ? "소개" : "About"}</Link><Link to="/publisher-message">{ko ? "발행인 인사말" : "Publisher's Message"}</Link></nav></div></div>
      </div>
