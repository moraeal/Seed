  ...
  return <article className="bg-paper pb-16">
    ...
    <div className="article-content-frame py-9 sm:py-12">
      ...
      <NumberedSection number="02" title={ko ? "시민의 삶에서 달라질 수 있는 두 가지" : "Two possible practical changes"}>
        ...
        <p className="mt-5 text-xs leading-6 text-charcoal/50">{bill.plenary_passed_at ? (ko ? "본회의 의결 내용입니다. 공포와 시행일은 별도로 확인해야 합니다." : "This reflects the plenary vote. Promulgation and the effective date require separate confirmation.") : (ko ? "아직 발의 단계입니다. 제안된 내용은 국회 심사 과정에서 바뀔 수 있습니다." : "This is a proposal. Its provisions may change during Assembly review.")}</p>
      </NumberedSection>
      ...
      {bill.media_coverage?.length > 0 && <NumberedSection number="07" title={ko ? "이 법안을 설명하는 언론 보도" : "News coverage explaining this bill"}>
        <p className="mb-5 text-sm leading-7 text-charcoal/60">{ko ? "법안명·의안번호·제안자와 발의일 또는 의결일을 대조해 같은 법안을 다룬 보도만 연결합니다. 기사 핵심은 요약해 소개하며, 오류가 확인되면 바로 수정하거나 제외합니다." : "Coverage is matched using the bill title, number, sponsor and proposal or vote date. Summaries are reviewed and corrected or removed when an error is found."}</p>
