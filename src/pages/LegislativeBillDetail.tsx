import { ArrowLeft, ArrowRight, CalendarDays, ExternalLink, Newspaper, Scale, Star } from "lucide-react";
...
export default function LegislativeBillDetail() {
  ...
  return <article className="bg-paper pb-16">
    <header className="border-b border-green-deep/15 bg-ivory py-9 sm:py-12"><div className="container-page max-w-5xl"><Link to="/monitoring/legislation" className="text-link text-xs"><ArrowLeft size={14}/>{ko ? "입법감시 목록" : "Legislative Watch"}</Link><div className="mt-4 border-t-2 border-navy pt-4"><div className="flex flex-wrap gap-2 text-xs font-extrabold text-green-deep"><span>{bill.committee || (ko ? "소관위 미정" : "Committee pending")}</span>{bill.bill_no && <span className="text-charcoal/45">{bill.bill_no}</span>}<span className="border border-green-deep/20 px-2 py-0.5">{bill.processing_result || bill.current_stage || (ko ? "본회의 의결" : "Plenary vote")}</span>{bill.is_featured && <span className="inline-flex items-center gap-1 text-gold"><Star size={13} fill="currentColor"/>{ko ? "씨앗이 주목하는 법안" : "Seed Voice selection"}</span>}</div><h1 className="article-detail-title mt-3">{title}</h1><p className="article-summary">{summary || (ko ? bill.official_summary : undefined) || (ko ? "국회 본회의를 통과한 법안의 내용과 영향을 검토합니다." : "This record examines the substance and effects of a plenary-passed bill.")}</p><div className="mt-4 flex flex-wrap items-center gap-4 border-t border-green-deep/10 pt-3 text-xs text-charcoal/50"><span className="flex items-center gap-1.5"><CalendarDays size={14}/>{ko ? "본회의 의결 " : "Plenary vote "}{(bill.plenary_passed_at || bill.proposed_date)?.replace(/-/g, ".") || "—"}</span><span>{bill.proposer || bill.representative_proposer}</span><span className="ml-auto flex items-center gap-1.5 font-bold text-green-deep"><Scale size={14}/>{ko ? `시민영향도 ${bill.importance_score}` : `Civic impact ${bill.importance_score}`}</span></div></div></div></header>
    ...
    <div className="article-content-frame py-9 sm:py-12">
      ...
      <NumberedSection number="02" title={ko ? "시민의 삶에서 크게 바뀌는 두 가지" : "The two biggest practical changes"}>
        ...
        <p className="mt-5 text-xs leading-6 text-charcoal/50">{ko ? "국회 본회의 의결 내용입니다. 정부 이송·공포와 시행일은 별도로 확인해야 합니다." : "This reflects the plenary vote. Government transfer, promulgation, and the effective date must be checked separately."}</p>
      </NumberedSection>
      ...
      <section className="article-section reading-column border-y border-green-deep/15 bg-ivory px-6 py-6 sm:px-8"><span className="section-kicker">04 · {ko ? "입법 취지" : "LEGISLATIVE RATIONALE"}</span><h2 className="mt-2 text-lg font-extrabold text-navy">{ko ? "왜 이 법안을 통과시켰나" : "Why did Parliament pass this bill?"}</h2><p className="mt-3 text-sm leading-7 text-charcoal/70">{shortOfficialExplanation(officialRationale, ko)}</p><p className="mt-3 text-xs leading-6 text-charcoal/45">{ko ? "국회에 제출된 제안 이유와 주요 내용을 독자가 읽기 쉽게 압축한 것입니다. 씨앗의 판단은 아래와 구분합니다." : "This condenses the official rationale and principal provisions for readers and remains separate from Seed Voice's view below."}</p></section>
      ...
      {bill.media_coverage?.length > 0 && <NumberedSection number="07" title={ko ? "이 법안을 설명하는 언론 보도" : "News coverage explaining this bill"}>
        <p className="mb-5 text-sm leading-7 text-charcoal/60">{ko ? "법안명·의안번호·제안자와 의결일을 대조해 같은 법안을 다룬 보도만 연결합니다. 기사 핵심은 자동으로 요약해 공개하며, 오류가 확인되면 바로 수정하거나 제외합니다." : "Coverage is matched using the bill title, number, sponsor, and vote date. Summaries are published automatically and corrected or removed when an error is found."}</p>
        <div className="divide-y divide-green-deep/15 border-y border-green-deep/15">{bill.media_coverage.map((item) => <a key={`${item.url}-${item.title}`} href={item.url} target="_blank" rel="noreferrer" className="group grid gap-3 py-5 sm:grid-cols-[1fr_auto] sm:items-start"><div><span className="inline-flex items-center gap-1.5 text-[11px] font-black text-green-deep"><Newspaper size={13}/>{item.source}{item.published_at ? ` · ${item.published_at.slice(0, 10).replace(/-/g, ".")}` : ""}</span><h3 className="mt-2 text-base font-extrabold leading-7 text-navy group-hover:text-green-mid">{item.title}</h3><p className="mt-2 text-sm leading-7 text-charcoal/65">{ko ? item.summary_ko : item.summary_en}</p></div><ExternalLink size={15} className="mt-1 text-green-deep"/></a>)}</div>
        {bill.media_checked_at && <p className="mt-3 text-xs text-charcoal/45">{ko ? `보도 확인 ${bill.media_checked_at.slice(0, 10).replace(/-/g, ".")}` : `Coverage checked ${bill.media_checked_at.slice(0, 10)}`}</p>}
      </NumberedSection>}

      <NumberedSection number={bill.media_coverage?.length ? "08" : "07"} title={ko ? "법안 진행 타임라인" : "Legislative timeline"}>
