            ...
            <section>
              ...
              <div className="mt-4 overflow-x-auto border-t-2 border-navy">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>{members.map((item) => <tr key={item.user_id} className="border-t border-green-deep/10"><td className="px-4 py-3 font-semibold text-navy">{item.nickname}<p className="mt-1 text-[11px] font-normal text-charcoal/45">{item.email}</p>{item.phone && <p className="mt-1 text-[11px] font-normal text-charcoal/45">{item.phone}</p>}<p className={`mt-1 text-[11px] font-bold ${item.content_subscription_consent ? "text-green-deep" : "text-charcoal/35"}`}>{item.content_subscription_consent ? (ko ? "이메일 콘텐츠 수신" : "Email content delivery") : (ko ? "수신 동의 없음" : "Not subscribed")}</p>{item.social_preferences.length > 0 && <p className="mt-1 text-[11px] font-normal text-charcoal/45">{ko ? "선택 채널" : "Selected channels"}: {item.social_preferences.map((channel) => channel === "kakao" ? (ko ? "카카오톡" : "KakaoTalk") : channel === "x" ? "X" : channel === "instagram" ? "Instagram" : "Telegram").join(" · ")}</p>}</td><td className="whitespace-nowrap px-4 py-3 text-xs text-charcoal/55">{formatDate(item.created_at)}<p className={`mt-1 font-bold ${item.email_confirmed_at ? "text-green-deep" : "text-amber-700"}`}>{item.email_confirmed_at ? (ko ? "인증 완료" : "Verified") : (ko ? "미인증" : "Unverified")}</p></td></tr>)}</tbody>
                </table>
