        ...
        <div className="container-page">
          ...
          <div className="grid gap-5 sm:gap-6 xl:grid-cols-[minmax(0,1.62fr)_minmax(340px,.82fr)] xl:items-stretch xl:gap-7">
            ...
            <aside className="divide-y divide-green-deep/15 border-y border-green-deep/20 xl:flex xl:h-full xl:flex-col xl:border-t-0" aria-label={ko ? "오늘의 핵심 콘텐츠" : "Today’s essential stories"}>
              {latestNews && <Link to={`/news/${latestNews.slug}`} className="group block py-3.5 sm:py-4 xl:flex-1 xl:pt-0"><p className="text-[9px] font-black tracking-[.14em] text-green-deep sm:text-[10px]">TODAY&apos;S NEWS</p><h2 className="editorial-title mt-1 break-keep text-[1.08rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-1.5 sm:text-xl xl:text-[1.25rem]">{latestNews.title}</h2><p className="mt-1 line-clamp-2 text-[12px] leading-5 text-charcoal/58 sm:mt-1.5 sm:text-[13px] sm:leading-5.5">{latestNews.summary}</p><span className="mt-2 inline-flex items-center gap-1 text-[11px] font-extrabold text-green-deep sm:mt-2.5 sm:text-xs">{ko ? "뉴스 읽기" : "Read news"}<ArrowRight size={13}/></span></Link>}
              {latestBriefing && <Link to={`/briefings/${latestBriefing.slug}`} className="group block py-3.5 sm:py-4 xl:flex-1"><p className="text-[9px] font-black tracking-[.14em] text-green-deep sm:text-[10px]">SEED CITIZEN BRIEFING</p><h2 className="editorial-title mt-1 break-keep text-[1.08rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-1.5 sm:text-xl xl:text-[1.25rem]">{latestBriefing.title}</h2><p className="mt-1 line-clamp-2 text-[12px] leading-5 text-charcoal/58 sm:mt-1.5 sm:text-[13px] sm:leading-5.5">{latestBriefing.summary}</p><span className="mt-2 inline-flex items-center gap-1 text-[11px] font-extrabold text-green-deep sm:mt-2.5 sm:text-xs">{ko ? "브리핑 읽기" : "Read briefing"}<ArrowRight size={13}/></span></Link>}
              {seedLanguageArticle && <Link to={`/seed-language/${seedLanguageArticle.slug}`} className="group block py-4 sm:py-5 xl:flex-1"><div className="flex items-center justify-between gap-3"><p className="text-[9px] font-black tracking-[.14em] text-green-deep sm:text-[10px]">SEED LANGUAGE</p><span className="text-[10px] font-extrabold text-green-deep/60">{ko ? "씨앗언어" : "Featured concept"}</span></div><div className="mt-2.5"><div>{ko ? <p className="flex items-baseline gap-2 text-2xl font-black tracking-[-.04em] text-green-deep sm:text-[2rem]"><span>{seedLanguageArticle.term}</span>{seedLanguageArticle.term === "민주" && <><span className="text-[.58em] font-bold tracking-[-.02em] text-green-deep/65">(民主)</span><span className="text-[.42em] font-black tracking-[.12em] text-charcoal/42">DEMOCRACY</span></>}</p> : <p className="editorial-title text-2xl font-black tracking-[-.05em] text-green-deep sm:text-3xl">{seedLanguageArticle.term}</p>}</div><h2 className="editorial-title mt-2 break-keep text-[1.02rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{seedLanguageArticle.title}</h2><p className="mt-1.5 text-[12px] leading-5 text-charcoal/58 sm:mt-1.5 sm:text-[13px] sm:leading-5.5">{ko ? "익숙한 말을 다시 묻습니다. 우리가 당연하게 사용하는 말 속의 전제와 권력을 살펴봅니다." : "We question familiar words again, examining the assumptions and power embedded in language we often take for granted."}</p><span className="mt-2.5 inline-flex items-center gap-1 text-[11px] font-extrabold text-green-deep sm:text-xs">{ko ? "씨앗언어 읽기" : "Read SEED Language"}<ArrowRight size={13}/></span></div></Link>}
            </aside>
          </div>
        </div>
      </section>

      <section className="py-9 sm:py-16" aria-labelledby="today-news-title"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">TODAY&apos;S NEWS</p><h2 id="today-news-title" className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "오늘의 뉴스" : "Today's News"}</h2><p className="mt-1.5 text-[12px] font-medium leading-5 text-charcoal/55 sm:text-sm sm:leading-6">{ko ? "사건보다 맥락을, 속보보다 의미를 전합니다." : "We put context before headlines and meaning before speed."}</p></div><Link to="/news" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div><div className="divide-y divide-green-deep/12 pt-1 md:grid md:grid-cols-3 md:gap-8 md:divide-y-0 md:pt-6">{news.slice(0, 3).map((item) => <article key={item.slug} className="group py-4 first:pt-3 md:py-0"><Link to={`/news/${item.slug}`} className="block"><div className="hidden h-[220px] overflow-hidden bg-ivory md:block lg:h-[240px]"><SafeImage src={resolveImageSrc(item.selectedNews.thumbnailUrl ?? item.heroImage.src)} alt={item.selectedNews.thumbnailAlt ?? item.heroImage.alt} referrerPolicy="no-referrer" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.018]" /></div><h3 className="editorial-title break-keep text-[1.08rem] font-bold leading-snug text-navy transition group-hover:text-green-mid md:mt-4 md:text-[1.35rem]">{item.title}</h3><p className="mt-1.5 line-clamp-2 text-[13px] leading-5 text-charcoal/58 md:mt-2 md:text-sm md:leading-6">{item.summary}</p><div className="mt-2 flex items-center gap-3 text-[11px] text-charcoal/45 sm:text-xs"><time>{item.date.replace(/-/g, ".")}</time><span className="inline-flex items-center gap-1"><Clock size={12}/>{item.readMinutes}{ko ? "분 읽기" : " min read"}</span></div></Link></article>)}</div></div></section>

      <section className="border-t border-green-deep/12 py-8 sm:py-12"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">SEED CITIZEN BRIEFING</p><h2 className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "시민브리핑" : "Citizen Briefing"}</h2><p className="mt-1.5 text-[12px] font-medium leading-5 text-charcoal/55 sm:text-sm sm:leading-6">{ko ? "시민에게는 때로 분노의 성명서보다 친절한 설명서가 필요합니다." : "Citizens sometimes need a clear explanation more than an angry statement."}</p></div><Link to="/briefings" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div><div className="grid gap-5 pt-4 sm:gap-6 sm:pt-4 lg:grid-cols-[.95fr_1.05fr]">{latestBriefing && <Link to={`/briefings/${latestBriefing.slug}`} className="group block">{latestBriefing.images?.[0] && <div className="overflow-hidden bg-green-deep"><SafeImage src={resolveImageSrc(latestBriefing.images[0].src)} alt={latestBriefing.images[0].alt} referrerPolicy="no-referrer" className="aspect-[16/7.1] w-full object-cover transition duration-500 group-hover:scale-[1.015] sm:aspect-[16/7.8]" /></div>}<h3 className="editorial-title mt-3 break-keep text-[1.3rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-3 sm:text-2xl">{latestBriefing.title}</h3><p className="mt-1.5 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-1.5 sm:text-sm sm:leading-6">{latestBriefing.summary}</p></Link>}<div className="divide-y divide-green-deep/15 border-y border-green-deep/15 lg:border-t-0">{briefings.slice(1, 5).map((briefing, index) => <Link key={briefing.slug} to={`/briefings/${briefing.slug}`} className="group grid grid-cols-[1.6rem_1fr] gap-2.5 py-3 sm:grid-cols-[2rem_1fr] sm:gap-3 sm:py-3"><span className="text-xs font-black text-green-deep/55 sm:text-sm">{String(index + 1).padStart(2, "0")}</span><div><h3 className="editorial-title break-keep text-[1rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{briefing.title}</h3><p className="mt-1 line-clamp-2 text-[12px] leading-5 text-charcoal/55 sm:line-clamp-2 sm:text-sm sm:leading-6">{briefing.summary}</p></div></Link>)}</div></div></div></section>

      <section className="border-t border-green-deep/12 py-8 sm:py-12"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">THE VOICE OF SEED</p><h2 className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "씨앗의 소리" : "Opinion & Ideas"}</h2><p className="mt-1.5 text-[12px] font-medium leading-5 text-charcoal/55 sm:text-sm sm:leading-6">{ko ? "정답을 말하기보다, 익숙한 생각에 질문을 던집니다." : "Rather than declare the answer, we question what has become familiar."}</p></div><Link to="/columns" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div><div className="grid gap-5 pt-4 sm:gap-6 sm:pt-4 lg:grid-cols-[.86fr_1.14fr] lg:items-stretch"><div className="h-full divide-y divide-green-deep/15 border-y border-green-deep/15 lg:border-t-0">{voiceListColumns.map((column, index) => <Link key={column.slug} to={`/columns/${column.slug}`} className="group grid grid-cols-[1.6rem_1fr] gap-2.5 py-3 sm:grid-cols-[2rem_1fr] sm:gap-3 sm:py-3"><span className="text-xs font-black text-green-deep/55 sm:text-sm">{String(index + 1).padStart(2, "0")}</span><div><h3 className="editorial-title break-keep text-[1rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{column.title}</h3><p className="mt-1 line-clamp-2 text-[12px] leading-5 text-charcoal/55 sm:line-clamp-2 sm:text-sm sm:leading-6">{column.summary}</p></div></Link>)}</div>{voiceLeadColumn && <Link to={`/columns/${voiceLeadColumn.slug}`} className="group flex h-full flex-col border-t border-green-deep/12 pt-4 lg:border-t-0 lg:pt-0"><div className="hidden overflow-hidden bg-ivory sm:block"><SafeImage src={resolveImageSrc(voiceLeadColumn.heroImage.src)} alt={voiceLeadColumn.heroImage.alt} referrerPolicy="no-referrer" className="aspect-[16/7.2] w-full object-cover transition duration-500 group-hover:scale-[1.015] lg:aspect-[16/5.8]" /></div><h3 className="editorial-title break-keep text-[1.2rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-3 sm:text-2xl">{voiceLeadColumn.title}</h3><p className="mt-1.5 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-1.5 sm:text-sm sm:leading-6">{voiceLeadColumn.summary}</p></Link>}</div></div></section>
