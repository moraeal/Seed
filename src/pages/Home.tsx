...
export default function Home() {
  ...
  return (
    <div className="bg-paper">
      ...
      <section className="border-y border-green-deep/12 bg-[#F6F5F0] py-8 sm:py-12" aria-labelledby="seed-language-home-title"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">SEED LANGUAGE</p><h2 id="seed-language-home-title" className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "씨앗언어" : "SEED Language"}</h2><p className="mt-1 text-[12px] text-charcoal/55 sm:mt-1.5 sm:text-sm">{ko ? "익숙한 말을 다시 묻습니다." : "We question familiar words again."}</p></div><Link to="/seed-language" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div>{seedLanguageArticle && <Link to={`/seed-language/${seedLanguageArticle.slug}`} className="group mt-4 grid gap-3 border border-green-deep/12 bg-white p-5 sm:mt-4 sm:gap-5 sm:p-7 lg:grid-cols-[.35fr_.65fr] lg:items-center"><div><p className="editorial-title text-4xl font-black tracking-[-.06em] text-green-deep sm:text-6xl">{seedLanguageArticle.term}</p></div><div><h3 className="editorial-title break-keep text-[1.25rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-3xl">{seedLanguageArticle.title}</h3><p className="mt-2 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-2 sm:text-base sm:leading-7">{seedLanguageArticle.summary}</p></div></Link>}</div></section>
      ...
      <section className="border-t border-green-deep/12 py-8 sm:py-12" aria-labelledby="publisher-home-title"><div className="container-page"><Link to="/publisher-message" className="group grid gap-5 border-y-[3px] border-navy py-6 sm:gap-7 sm:py-7 lg:grid-cols-[.38fr_.62fr] lg:items-center"><div><p className="section-kicker">PUBLISHER'S MESSAGE</p><p className="editorial-title mt-3 text-4xl font-black tracking-[-.06em] text-green-deep sm:mt-3 sm:text-5xl">{ko ? "한시언" : "Han Si-eon"}</p><p className="mt-1.5 text-[12px] font-semibold text-charcoal/50 sm:mt-2 sm:text-sm">{ko ? "씨앗의 소리 발행인" : "Publisher, SEED VOICE"}</p></div><div><h2 id="publisher-home-title" className="editorial-title break-keep text-[1.55rem] font-bold leading-tight text-navy transition group-hover:text-green-mid sm:text-3xl">{ko ? "어느 작은 시민의 말" : "The words of one ordinary citizen"}</h2><p className="mt-3 line-clamp-4 text-[13px] leading-6 text-charcoal/65 sm:mt-3 sm:max-w-3xl sm:text-base sm:leading-7">{ko ? "유명인이나 전문가의 권위가 아니라 한 시민의 질문과 판단으로 세상을 바라보겠습니다. 사실을 확인하고 자유와 책임의 기준으로 권력을 살피며, 완성된 답보다 질문하고 수정하며 성장하는 시민의 목소리를 기록합니다." : "SEED VOICE looks at the world through one citizen’s questions and judgment rather than borrowed authority. It verifies facts, scrutinizes power through freedom and responsibility, and records a civic voice willing to question, correct, and grow."}</p><span className="mt-3 inline-flex items-center gap-1 text-[12px] font-extrabold text-green-deep sm:mt-4 sm:text-sm">{ko ? "발행인의 말 읽기" : "Read the publisher's message"}<ArrowRight size={15}/></span></div></Link></div></section>
