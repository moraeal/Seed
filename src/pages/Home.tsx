...
export default function Home() {
  ...
  return (
    <div className="bg-paper">
        ...
        <div className="container-page">
          ...
          <div className="grid gap-5 sm:gap-6 xl:grid-cols-[minmax(0,1.62fr)_minmax(340px,.82fr)] xl:gap-7">
            ...
            <aside className="divide-y divide-green-deep/15 border-y border-green-deep/20 xl:border-t-0" aria-label={ko ? "오늘의 핵심 콘텐츠" : "Today’s essential stories"}>
              ...
              {seedLanguageArticle && <Link to={`/seed-language/${seedLanguageArticle.slug}`} className="group block py-3.5 sm:py-4"><p className="text-[9px] font-black tracking-[.14em] text-green-deep sm:text-[10px]">SEED LANGUAGE</p><div className="mt-1 grid grid-cols-[auto_1fr] items-start gap-2.5 sm:mt-1.5 sm:gap-3"><span className="editorial-title text-2xl font-black tracking-[-.05em] text-green-deep sm:text-3xl">{seedLanguageArticle.term}</span><div><h2 className="editorial-title break-keep text-[1rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{seedLanguageArticle.title}</h2><p className="mt-1 line-clamp-2 text-[12px] leading-5 text-charcoal/58 sm:mt-1.5 sm:text-[13px] sm:leading-5.5">{seedLanguageArticle.summary}</p></div></div></Link>}
            </aside>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-green-deep/12 py-9 sm:py-16"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">SEED CITIZEN BRIEFING</p><h2 className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "시민브리핑" : "Citizen Briefing"}</h2></div><Link to="/briefings" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div><div className="grid gap-5 pt-4 sm:gap-8 sm:pt-6 lg:grid-cols-[1.15fr_.85fr]">{latestBriefing && <Link to={`/briefings/${latestBriefing.slug}`} className="group block">{latestBriefing.images?.[0] && <div className="overflow-hidden bg-green-deep"><SafeImage src={resolveImageSrc(latestBriefing.images[0].src)} alt={latestBriefing.images[0].alt} referrerPolicy="no-referrer" className="aspect-[16/8.8] w-full object-cover transition duration-500 group-hover:scale-[1.015] sm:aspect-[16/9.5]" /></div>}<h3 className="editorial-title mt-3 break-keep text-[1.3rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-4 sm:text-3xl">{latestBriefing.title}</h3><p className="mt-1.5 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-2 sm:text-base sm:leading-7">{latestBriefing.summary}</p></Link>}<div className="divide-y divide-green-deep/15 border-y border-green-deep/15 lg:border-t-0">{briefings.slice(1, 5).map((briefing, index) => <Link key={briefing.slug} to={`/briefings/${briefing.slug}`} className="group grid grid-cols-[1.6rem_1fr] gap-2.5 py-3.5 sm:grid-cols-[2rem_1fr] sm:gap-3 sm:py-4"><span className="text-xs font-black text-green-deep/55 sm:text-sm">{String(index + 1).padStart(2, "0")}</span><div><h3 className="editorial-title break-keep text-[1rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{briefing.title}</h3><p className="mt-1 line-clamp-1 text-[12px] leading-5 text-charcoal/55 sm:line-clamp-2 sm:text-sm sm:leading-6">{briefing.summary}</p></div></Link>)}</div></div></div></section>

      <section className="border-t border-green-deep/12 py-9 sm:py-16"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">THE VOICE OF SEED</p><h2 className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "씨앗의 소리" : "Opinion & Ideas"}</h2></div><Link to="/columns" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div><div className="grid gap-5 pt-4 sm:gap-8 sm:pt-6 lg:grid-cols-[.86fr_1.14fr] lg:items-start"><div className="divide-y divide-green-deep/15 border-y border-green-deep/15 lg:border-t-0">{voiceListColumns.map((column, index) => <Link key={column.slug} to={`/columns/${column.slug}`} className="group grid grid-cols-[1.6rem_1fr] gap-2.5 py-3.5 sm:grid-cols-[2rem_1fr] sm:gap-3 sm:py-4"><span className="text-xs font-black text-green-deep/55 sm:text-sm">{String(index + 1).padStart(2, "0")}</span><div><h3 className="editorial-title break-keep text-[1rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-lg">{column.title}</h3><p className="mt-1 line-clamp-1 text-[12px] leading-5 text-charcoal/55 sm:line-clamp-2 sm:text-sm sm:leading-6">{column.summary}</p></div></Link>)}</div>{voiceLeadColumn && <Link to={`/columns/${voiceLeadColumn.slug}`} className="group block border-t border-green-deep/12 pt-4 lg:border-t-0 lg:pt-0"><div className="hidden overflow-hidden bg-ivory sm:block"><SafeImage src={resolveImageSrc(voiceLeadColumn.heroImage.src)} alt={voiceLeadColumn.heroImage.alt} referrerPolicy="no-referrer" className="aspect-[16/9.5] w-full object-cover transition duration-500 group-hover:scale-[1.015]" /></div><h3 className="editorial-title break-keep text-[1.2rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-4 sm:text-3xl">{voiceLeadColumn.title}</h3><p className="mt-1.5 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-2 sm:text-base sm:leading-7">{voiceLeadColumn.summary}</p></Link>}</div></div></section>

      <section className="border-y border-green-deep/12 bg-[#F6F5F0] py-9 sm:py-16" aria-labelledby="seed-language-home-title"><div className="container-page"><div className="flex items-end justify-between gap-3 border-b-[3px] border-navy pb-2.5 sm:gap-4 sm:pb-3"><div><p className="section-kicker">SEED LANGUAGE</p><h2 id="seed-language-home-title" className="editorial-title mt-1 text-[1.45rem] font-bold text-navy sm:mt-1.5 sm:text-3xl">{ko ? "씨앗언어" : "SEED Language"}</h2><p className="mt-1 text-[12px] text-charcoal/55 sm:mt-2 sm:text-sm">{ko ? "익숙한 말을 다시 묻습니다." : "We question familiar words again."}</p></div><Link to="/seed-language" className="text-link shrink-0 text-xs sm:text-sm">{ko ? "전체보기" : "View all"}<ArrowRight size={14}/></Link></div>{seedLanguageArticle && <Link to={`/seed-language/${seedLanguageArticle.slug}`} className="group mt-4 grid gap-3 border border-green-deep/12 bg-white p-5 sm:mt-6 sm:gap-6 sm:p-8 lg:grid-cols-[.35fr_.65fr] lg:items-center"><div><p className="editorial-title text-4xl font-black tracking-[-.06em] text-green-deep sm:text-6xl">{seedLanguageArticle.term}</p></div><div><h3 className="editorial-title break-keep text-[1.25rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:text-3xl">{seedLanguageArticle.title}</h3><p className="mt-2 line-clamp-2 text-[13px] leading-5.5 text-charcoal/60 sm:mt-3 sm:text-base sm:leading-7">{seedLanguageArticle.summary}</p></div></Link>}</div></section>

      <PopularLatest />

      <section className="border-t border-green-deep/12 py-9 sm:py-16" aria-labelledby="newcomer-title"><div className="container-page"><div className="max-w-2xl"><p className="section-kicker">START HERE</p><h2 id="newcomer-title" className="editorial-title mt-1.5 text-[1.55rem] font-bold text-navy sm:mt-2 sm:text-4xl">{ko ? "처음 오셨다면" : "New to SEED VOICE?"}</h2><p className="mt-2 text-[13px] leading-6 text-charcoal/60 sm:mt-3 sm:text-base sm:leading-7">{ko ? "씨앗의 소리가 무엇을 보고 어떤 기준으로 판단하는지, 아래 세 글에서 가장 빠르게 확인할 수 있습니다." : "These three pages are the fastest way to understand what SEED VOICE watches and the standards it uses."}</p></div><div className="mt-5 grid gap-3 sm:mt-7 sm:gap-5 md:grid-cols-3">{newcomerLinks.map((item, index) => <Link key={item.to} to={item.to} className="group flex flex-col justify-between border border-green-deep/12 bg-white p-5 transition hover:-translate-y-0.5 hover:border-green-deep/30 sm:min-h-[220px] sm:p-6"><div><div className="flex items-center justify-between gap-3"><p className="text-[9px] font-black tracking-[.14em] text-green-deep sm:text-[10px]">{item.kicker}</p><span className="text-[11px] font-black text-charcoal/25 sm:text-xs">0{index + 1}</span></div><h3 className="editorial-title mt-3 break-keep text-[1.18rem] font-bold leading-snug text-navy transition group-hover:text-green-mid sm:mt-5 sm:text-2xl">{item.title}</h3><p className="mt-2 line-clamp-2 text-[13px] leading-5.5 text-charcoal/58 sm:mt-3 sm:text-sm sm:leading-6">{item.summary}</p></div><span className="mt-3 inline-flex items-center gap-1 text-[11px] font-extrabold text-green-deep sm:mt-6 sm:text-xs">{ko ? "읽어보기" : "Read"}<ArrowRight size={13}/></span></Link>)}</div></div></section>

      <section className="border-t border-green-deep/12 py-9 sm:py-16" aria-labelledby="publisher-home-title"><div className="container-page"><Link to="/publisher-message" className="group grid gap-5 border-y-[3px] border-navy py-6 sm:gap-8 sm:py-10 lg:grid-cols-[.38fr_.62fr] lg:items-center"><div><p className="section-kicker">PUBLISHER'S MESSAGE</p><p className="editorial-title mt-3 text-4xl font-black tracking-[-.06em] text-green-deep sm:mt-4 sm:text-6xl">{ko ? "한시언" : "Han Si-eon"}</p><p className="mt-1.5 text-[12px] font-semibold text-charcoal/50 sm:mt-2 sm:text-sm">{ko ? "씨앗의 소리 발행인" : "Publisher, SEED VOICE"}</p></div><div><h2 id="publisher-home-title" className="editorial-title break-keep text-[1.55rem] font-bold leading-tight text-navy transition group-hover:text-green-mid sm:text-4xl">{ko ? "어느 작은 시민의 말" : "The words of one ordinary citizen"}</h2><p className="mt-3 line-clamp-4 text-[13px] leading-6 text-charcoal/65 sm:mt-4 sm:max-w-3xl sm:text-lg sm:leading-8">{ko ? "유명인이나 전문가의 권위가 아니라 한 시민의 질문과 판단으로 세상을 바라보겠습니다. 사실을 확인하고 자유와 책임의 기준으로 권력을 살피며, 완성된 답보다 질문하고 수정하며 성장하는 시민의 목소리를 기록합니다." : "SEED VOICE looks at the world through one citizen’s questions and judgment rather than borrowed authority. It verifies facts, scrutinizes power through freedom and responsibility, and records a civic voice willing to question, correct, and grow."}</p><span className="mt-3 inline-flex items-center gap-1 text-[12px] font-extrabold text-green-deep sm:mt-5 sm:text-sm">{ko ? "발행인의 말 읽기" : "Read the publisher's message"}<ArrowRight size={15}/></span></div></Link></div></section>
