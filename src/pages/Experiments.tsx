...
import { experimentTranslations } from "../data/contentTranslations/staticPrograms";
...
export default function Experiments() {
  ...
  const items = experimentItems.map((item, index) => ko ? item : { ...item, ...experimentTranslations[index] });
      ...
      <header className="border-b border-green-deep/15 bg-ivory">
        <div className="container-page grid gap-8 py-14 sm:py-20 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
          <div>
            ...
            <h1 className="editorial-title mt-4 text-5xl font-bold text-navy sm:text-6xl">{ko ? "시민실험" : "Civic Experiments"}</h1>
          </div>
          <p className="max-w-2xl text-base leading-8 text-charcoal/65">
            {ko ? "큰 구호보다 작고 검증 가능한 해법부터 시험합니다. 무엇이 효과가 있었고 무엇이 실패했는지 공개 기록으로 남깁니다." : "We test small, verifiable solutions before making large claims. What works, what fails and what changes are recorded publicly so others can learn from the experiment."}
          </p>
        </div>
      </header>
