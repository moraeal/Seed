...
export default function NewsDetail() {
  ...
  const detailHeroImage = isLhArticle
    ? {
        ...
        alt: ko ? "LH 분리 추진을 상징적으로 표현한 삽화" : "Conceptual illustration of the proposed LH split",
        caption: "",
        credit: "",
        sourceUrl: undefined,
      }
  ...
  return <article className="bg-paper">
    ...
    <div className="article-content-frame py-8 sm:py-12">
      ...
      <div className={article.slug === "supreme-court-renomination-standoff-2026" ? "reading-column" : undefined}>
        <SourceArticleCard news={selectedNews} ko={ko}/>
      </div>
      {article.slug === "supreme-court-renomination-standoff-2026" && <Link to="/monitoring/supreme-court-renomination-tracker-2026" className="mb-10 flex items-center justify-between gap-4 border-l-4 border-gold bg-navy px-5 py-5 text-white transition hover:bg-green-deep sm:px-7"><span><span className="text-[10px] font-black tracking-[.14em] text-gold">NEWS TRACKER</span><strong className="mt-1 block text-base leading-7">{ko ? "대법관 제청을 되돌릴 수 있나 — 날짜별 진행 기록 보기" : "Can the nomination be sent back? Follow the dated case record"}</strong></span><ArrowRight className="shrink-0" size={19}/></Link>}
