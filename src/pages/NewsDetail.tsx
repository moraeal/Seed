...
export default function NewsDetail() {
  ...
  return <article className="bg-paper">
    <header className="border-b border-green-deep/15 bg-ivory py-12 sm:py-20">
      <div className="container-page max-w-5xl"><Link to="/news" className="text-link"><ArrowLeft size={16}/>{ko ? "씨드뉴스 목록" : "SEED News"}</Link><div className="mt-10 border-t-2 border-navy pt-8"><h1 className="editorial-title max-w-4xl text-4xl font-bold leading-[1.1] text-navy sm:text-6xl">{article.title}</h1><p className="mt-7 max-w-3xl text-base leading-8 text-charcoal/60">{article.summary}</p></div><div className="mt-9 flex flex-wrap items-center gap-5 border-t border-green-deep/10 pt-5 text-xs text-charcoal/45"><time>{article.date.replace(/-/g, ".")}</time><span className="flex items-center gap-1"><Clock size={14}/>{ko ? `읽는 시간 ${article.readMinutes}분` : `${article.readMinutes} min read`}</span><button onClick={share} className="button-secondary ml-auto min-h-9 px-3 py-2 text-xs"><Share2 size={15}/>{ko ? "공유" : "Share"}</button></div></div>
    </header>
