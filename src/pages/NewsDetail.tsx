...
export default function NewsDetail() {
  ...
  return <article className="bg-paper">
    <header className="border-b border-green-deep/15 bg-ivory py-8 sm:py-12">
      <div className="container-page max-w-5xl"><Link to="/news" className="text-link"><ArrowLeft size={16}/>{ko ? "오늘의뉴스 목록" : "Today's News"}</Link><div className="mt-6 border-t-2 border-navy pt-5"><h1 className="editorial-title max-w-4xl text-[1.6rem] font-bold leading-[1.15] text-navy sm:text-[2.625rem]">{article.title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-charcoal/60">{article.summary}</p></div><div className="mt-5 flex flex-wrap items-center gap-4 border-t border-green-deep/10 pt-3 text-xs text-charcoal/45"><time>{article.date.replace(/-/g, ".")}</time><span className="flex items-center gap-1"><Clock size={14}/>{ko ? `읽는 시간 ${article.readMinutes}분` : `${article.readMinutes} min read`}</span><button onClick={share} className="button-secondary ml-auto min-h-9 px-3 py-2 text-xs"><Share2 size={15}/>{ko ? "공유" : "Share"}</button></div></div>
    </header>
