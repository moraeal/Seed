  ...
  return <section className="min-h-[70vh] bg-paper pb-16">
    ...
    <div className="container-page py-8 sm:py-10">
      <nav aria-label={ko ? "세금감시 목록 바로가기" : "Tax watch list shortcuts"} className="mb-7 grid overflow-hidden rounded-xl border border-charcoal/10 bg-white shadow-[0_8px_24px_rgba(31,51,73,0.045)] sm:grid-cols-3">
        <a href="#tax-commentary-list" className="flex items-center justify-between border-b-2 border-green-deep px-5 py-4 text-sm font-black text-navy transition hover:bg-[#FAF9F5] sm:text-base">{ko ? "세금감시 기사" : "Watch Articles"}<span className="rounded-full bg-charcoal/5 px-2 py-0.5 text-xs text-charcoal/55">{taxCommentaries.length}</span></a>
        <a href="#today-tax-policies" className="flex items-center justify-between border-b-2 border-transparent px-5 py-4 text-sm font-black text-navy transition hover:border-charcoal/20 hover:bg-[#FAF9F5] sm:text-base">{ko ? "오늘의 세금정책" : "Today's Tax Policies"}<span className="rounded-full bg-charcoal/5 px-2 py-0.5 text-xs text-charcoal/55">{todayPolicies.length}</span></a>
        <a href="#past-tax-policies" className="flex items-center justify-between border-b-2 border-transparent px-5 py-4 text-sm font-black text-navy transition hover:border-charcoal/20 hover:bg-[#FAF9F5] sm:text-base">{ko ? "지난 세금정책" : "Past Tax Policies"}<span className="rounded-full bg-charcoal/5 px-2 py-0.5 text-xs text-charcoal/55">{pastPolicies.length}</span></a>
      </nav>
