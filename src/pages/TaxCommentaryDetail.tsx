...
export default function TaxCommentaryDetail() {
  ...
  const renderBodyImage = (sectionIndex: number) => article.bodyImage?.afterSection === sectionIndex && <InteractiveFigure src={imageSrc(article.bodyImage.src)} alt={article.bodyImage.alt[ko ? "ko" : "en"]} caption={article.bodyImage.caption[ko ? "ko" : "en"]} credit={ko ? "씨앗의 소리 AI 제작 이미지" : "AI-generated editorial image by SEED VOICE"} figureClassName="my-12 overflow-hidden border border-green-deep/10 bg-white shadow-[0_22px_65px_rgba(23,76,58,.1)]" imageClassName="aspect-[16/9] w-full object-cover" />;
