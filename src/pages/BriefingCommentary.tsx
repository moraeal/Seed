...
export default function BriefingCommentary() {
  ...
  return <article className="bg-paper">
    <header className="border-b border-green-deep/10 bg-green-deep py-8 text-white sm:py-11">
      <div className="container-page max-w-4xl">
        ...
        <span className="mt-6 block text-[11px] font-extrabold tracking-[0.18em] text-gold-light">SEED SPECIAL COMMENTARY</span>
        <p className="mt-3 text-sm font-bold text-white/60">{ko ? "씨드 특별논평" : "SEED Commentary"}</p>
        <h1 className="mt-3 text-[1.3rem] font-extrabold leading-tight sm:text-[2.1rem]">{briefing.commentary.title}</h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-white/75 sm:text-[17px]">{briefing.commentary.summary}</p>
        <div className="mt-5 flex flex-wrap items-center gap-4 text-xs text-white/55"><span>{briefing.author}</span><time>{briefing.date.replace(/-/g, ".")} {ko ? "기준" : "as of"}</time>{briefing.pdfPath && <a href={`${import.meta.env.BASE_URL}${briefing.pdfPath}`} download className="button-light ml-auto min-h-9 px-3 py-2 text-xs"><Download size={15} />{ko ? "PDF 원문 내려받기" : "Download PDF"}</a>}</div>
      </div>
    </header>
