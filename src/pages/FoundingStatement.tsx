...
export default function FoundingStatement() {
  ...
  const sections = ko
    ? [
        ...
        ["씨앗처럼 자라고 연결되는 시민을 기다립니다", "씨앗은 이미 생명을 품고 있지만 아직 완성된 존재는 아닙니다. 시민도 관계 속에서 배우고 책임을 감당하며 더 나은 시민으로 자랍니다. 씨앗의 소리는 완성된 답을 가진 사람들의 조직이 아니라, 함께 질문하고 배우며 행동하는 동료시민의 플랫폼이 되고자 합니다. 우리의 첫 방향과 신뢰, 활동의 기반을 함께 세울 창립파트너를 기다립니다."],
      ]
    : [
        ...
        ["We Welcome Citizens Who Grow and Connect Like Seeds", "A seed already carries life, but it is not complete. Citizens also learn through relationships, accept responsibility, and grow toward fuller civic capacity. SEED VOICE seeks to be a platform not for people who claim to possess all the answers, but for fellow citizens who question, learn, and act together. We invite Founding Partners to help establish our first direction, trust, and foundation for action."],
      ];

  return (
    <article className="bg-paper pb-20 sm:pb-28">
      <header className="border-b border-green-deep/15 bg-ivory py-14 sm:py-20">
        <div className="container-page max-w-5xl"><Link to="/about" className="text-link"><ArrowLeft size={16}/>{ko ? "저널 소개" : "About the Journal"}</Link><span className="section-kicker mt-10 block">FOUNDING STATEMENT</span><h1 className="editorial-title mt-4 text-4xl font-bold leading-tight text-navy sm:text-6xl">{ko ? "씨앗의 소리 창립취지문" : "Founding Statement of SEED VOICE"}</h1><p className="mt-6 max-w-3xl text-lg font-semibold leading-8 text-charcoal/65">{ko ? "자유로운 시민, 책임 있는 공익, 국가와 시장을 바로 세우는 강한 시민사회를 향해" : "Toward free citizens, accountable public purpose, and a strong civil society that keeps state and market in balance"}</p></div>
      </header>
      ...
      <div className="container-page max-w-5xl py-12 sm:py-16">
        ...
        <footer className="mt-16 border-t-2 border-navy pt-9"><p className="text-sm font-extrabold tracking-[.08em] text-green-deep">{ko ? "2026년 9월 · 씨앗의 소리 창립준비위원회" : "September 2026 · Founding Committee, SEED VOICE"}</p><Link to="/about" className="button-secondary mt-7">{ko ? "저널 소개로 돌아가기" : "Back to the Journal"}</Link></footer>
      </div>
    </article>
  );
}
