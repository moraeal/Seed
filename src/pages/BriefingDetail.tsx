...
export default function BriefingDetail() {
  ...
  return <article className="bg-paper">
    <header className="border-b border-green-deep/10 bg-ivory py-12 sm:py-16"><div className="container-page max-w-4xl"><Link to="/briefings" className="text-link"><ArrowLeft size={16} />시민브리핑 목록</Link><span className="section-kicker mt-9 block">{briefing.category}</span><h1 className="mt-4 text-3xl font-extrabold leading-tight text-navy sm:text-5xl">{briefing.title}</h1><p className="mt-5 text-base leading-8 text-charcoal/65 sm:text-lg">{briefing.summary}</p><div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-3 text-xs text-charcoal/45"><span>{briefing.author}</span><time>{briefing.date.replace(/-/g, ".")} 기준</time><span className="flex items-center gap-1"><Clock size={14} />읽는 시간 {briefing.readMinutes}분</span><div className="flex w-full gap-2 sm:ml-auto sm:w-auto"><button onClick={share} className="button-secondary min-h-9 px-3 py-2 text-xs"><Share2 size={15} />공유</button>{briefing.pdfPath && <a href={`${import.meta.env.BASE_URL}${briefing.pdfPath}`} download className="button-primary min-h-9 px-3 py-2 text-xs"><Download size={15} />PDF 원문 내려받기</a>}</div></div></div></header>
