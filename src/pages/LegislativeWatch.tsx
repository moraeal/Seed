      ...
      <section id="legislative-commentary-list" className="scroll-mt-24 overflow-hidden rounded-xl border border-charcoal/10 bg-white shadow-[0_12px_32px_rgba(31,51,73,0.055)]" aria-labelledby="legislative-commentary-list-title">
        ...
        <div className="grid lg:grid-cols-2">
          {linkedLegislativeColumnCommentaries.map((article) => {
            ...
            return <Link key={article.slug} to={article.href} className="group grid gap-4 border-t border-charcoal/10 p-5 transition hover:bg-[#FBFAF6] sm:grid-cols-[180px_1fr] sm:items-center sm:p-6 lg:odd:border-r">
              ...
              <div className="flex min-w-0 flex-col"><div className="flex flex-wrap items-center gap-2 text-[11px] font-extrabold text-charcoal/55"><span>{ko ? "입법 논평" : "COMMENTARY"}</span><span className="text-charcoal/35">{article.billLabel[ko ? "ko" : "en"]}</span></div><h3 className="editorial-title mt-2 line-clamp-2 text-xl font-bold leading-snug text-navy transition group-hover:text-green-deep">{edition.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-charcoal/60">{edition.summary}</p><div className="mt-auto flex items-center gap-3 border-t border-charcoal/10 pt-3 text-xs text-charcoal/45"><time>{article.date.replace(/-/g, ".")}</time><span className="flex items-center gap-1"><Clock size={12}/>{article.readMinutes}{ko ? "분" : " min"}</span><span className="ml-auto flex items-center gap-1.5 font-extrabold text-green-deep">{ko ? "논평 읽기" : "Read"}<ArrowRight size={13}/></span></div></div>
            </Link>;
          })}
          {legislativeCommentaries.map((article) => {
            const edition = getLegislativeCommentaryEdition(article, ko ? "ko" : "en");
            return <Link key={article.slug} to={`/monitoring/legislation/commentary/${article.slug}`} className="group grid gap-4 border-t border-charcoal/10 p-5 transition hover:bg-[#FBFAF6] sm:grid-cols-[180px_1fr] sm:items-center sm:p-6 lg:odd:border-r">
              <div className="overflow-hidden bg-ivory"><SafeImage src={`${import.meta.env.BASE_URL}${article.heroSrc}`} alt={edition.heroAlt} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-[1.025]" /></div>
              <div className="flex min-w-0 flex-col"><div className="flex flex-wrap items-center gap-2 text-[11px] font-extrabold text-charcoal/55"><span>{ko ? "입법 논평" : "COMMENTARY"}</span><span className="text-charcoal/35">{ko ? `의안 ${article.billNo}` : `Bill ${article.billNo}`}</span></div><h3 className="editorial-title mt-2 line-clamp-2 text-xl font-bold leading-snug text-navy transition group-hover:text-green-deep">{edition.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-charcoal/60">{edition.summary}</p><div className="mt-auto flex items-center gap-3 border-t border-charcoal/10 pt-3 text-xs text-charcoal/45"><time>{article.date.replace(/-/g, ".")}</time><span className="flex items-center gap-1"><Clock size={12}/>{article.readMinutes}{ko ? "분" : " min"}</span><span className="ml-auto flex items-center gap-1.5 font-extrabold text-green-deep">{ko ? "논평 읽기" : "Read"}<ArrowRight size={13}/></span></div></div>
            </Link>;
          })}
        </div>
      </section>
