...
export default function TaxPolicyDetail() {
  ...
  return <article className="bg-paper pb-16">
    ...
    <div className="article-content-frame py-9 sm:py-12">
      ...
      <section className="article-section reading-column border-y border-green-deep/15 bg-ivory px-6 py-6 sm:px-8"><span className="section-kicker">04 · {ko ? "정부·발의자 설명" : "OFFICIAL RATIONALE"}</span><h2 className="mt-2 text-lg font-extrabold text-navy">{ko ? "왜 이 정책을 내놓았나" : "Why was this policy proposed?"}</h2><p className="mt-3 text-sm leading-7 text-charcoal/70">{localized(policy.officialRationale, ko)}</p><p className="mt-3 text-xs leading-6 text-charcoal/45">{ko ? "공식 문서의 제안 취지를 독자가 읽기 쉽게 압축한 것입니다. 씨앗의 판단은 아래와 구분합니다." : "This is a reader-friendly condensation of the rationale in the official document and is separate from Seed Voice's view below."}</p></section>
