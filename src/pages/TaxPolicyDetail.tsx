...
export default function TaxPolicyDetail() {
  ...
  return <article className="bg-paper pb-16">
    ...
    <div className="article-content-frame py-9 sm:py-12">
      <figure className="article-visual-wide overflow-hidden bg-navy"><SafeImage src={ko ? policy.heroImage.ko : policy.heroImage.en} alt={localized(policy.heroImage.alt, ko)} className="w-full" loading="eager"/><figcaption className="bg-white px-4 py-3 text-xs leading-6 text-charcoal/50">{localized(policy.heroImage.alt, ko)} · {ko ? "행정안전부 발표자료를 바탕으로 씨앗의 소리가 재구성했습니다." : "Reconstructed by Seed Voice from the Interior Ministry's official plan."}</figcaption></figure>
      ...
      <NumberedSection number="02" title={ko ? "시민의 삶에서 크게 바뀌는 두 가지" : "The two biggest practical changes"}>
        ...
        <p className="mt-5 text-xs leading-6 text-charcoal/50">{ko ? "아직 확정된 법률이 아닙니다. 입법예고와 국회 심사 과정에서 대상과 범위가 달라질 수 있습니다." : "This is not yet enacted law. Its scope may change during public consultation and parliamentary review."}</p>
      </NumberedSection>
      ...
      <NumberedSection number="07" title={ko ? "정책 진행 타임라인" : "Policy timeline"}>
        <ol className="space-y-5 border-l-2 border-green-deep/20 pl-6">{policy.timeline.map((event, index) => <li key={`${event.date}-${event.title.ko}`}><time className="text-xs font-black text-green-deep">{event.date.length === 7 ? event.date.replace("-", ".") : dateText(event.date)}</time><h3 className="mt-1 font-bold text-navy">{localized(event.title, ko)}</h3>{index === 2 && <span className="mt-1 inline-flex bg-green-pale px-2 py-1 text-[10px] font-bold text-green-deep">{ko ? "현재 확인 단계" : "Current verified stage"}</span>}</li>)}</ol>
      </NumberedSection>
