import { ArrowRight, Check, Mail, Network, ShieldCheck, Sprout, UserRound, Users } from "lucide-react";
import { Link } from "react-router-dom";
import { AUDITION_URL, CONTACT_EMAIL, CONTACT_URL, getContent } from "../data/siteContent";
    ...
    <>
        ...
        <div className="container-page grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href={AUDITION_URL} target="_blank" rel="noopener noreferrer" className="button-primary" aria-label={language === "en" ? "Join citizen proposals" : "시민제안 참여하기"}>
          ...
          <div className="rounded-lg bg-green-deep p-7 text-white shadow-soft sm:p-10">
            ...
            <p className="mt-8 text-xs font-extrabold tracking-[0.14em] text-gold">SEED PRINCIPLES</p>
      ...
      <section className="section-band bg-paper">
        <div className="container-page grid gap-5 lg:grid-cols-3">
          <article className="content-card lg:col-span-1">
            ...
            <p className="mt-5 rounded-md bg-green-pale p-4 text-sm font-semibold leading-7 text-green-deep">{t.atlasAlignment}</p>
          </article>
          <article className="content-card lg:col-span-1">
      ...
      <section className="section-band bg-[#F4F5F2]">
        <div className="container-page">
          ...
          <h2 className="mt-3 text-3xl font-extrabold text-navy">{t.organizationProfileTitle}</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {t.organizationProfile.map(([label, value]) => (
              <article key={label} className="content-card">
                <h3 className="text-sm font-extrabold text-green-mid">{label}</h3>
                {value === CONTACT_EMAIL ? (
                  <a href={CONTACT_URL} className="mt-3 block break-words text-lg font-extrabold text-navy underline decoration-green-mid/30 underline-offset-4">
                    {value}
                  </a>
                ) : (
                  <p className="mt-3 text-lg font-extrabold leading-7 text-navy">{value}</p>
                )}
              </article>
            ))}
          </div>
          <p className="mt-6 rounded-lg border border-green-deep/10 bg-paper p-5 text-sm leading-7 text-charcoal/65">{t.organizationProfileNote}</p>
        </div>
      </section>
      ...
      <section className="section-band bg-navy text-white">
        <div className="container-page grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <article className="rounded-lg border border-white/10 bg-white/[0.06] p-8 sm:p-10">
            <UserRound className="text-gold" size={30} />
            <p className="mt-5 text-xs font-extrabold tracking-[0.14em] text-gold">{t.founderRole}</p>
            <h2 className="mt-3 text-3xl font-extrabold">{t.founderName}</h2>
            <p className="mt-5 text-sm leading-7 text-white/70">{t.founderBio}</p>
          </article>
          <article className="rounded-lg border border-white/10 bg-white/[0.06] p-8 sm:p-10">
            <Users className="text-gold" size={30} />
            <p className="mt-5 text-xs font-extrabold tracking-[0.14em] text-gold">{t.leadershipTitle}</p>
            <h2 className="mt-3 text-3xl font-extrabold">{t.leadershipApproachTitle}</h2>
            <p className="mt-5 text-sm leading-7 text-white/70">{t.leadership}</p>
          </article>
        </div>
      </section>
      ...
      <section className="section-band bg-paper">
        <div className="container-page grid gap-5 lg:grid-cols-2">
          <article className="content-card">
            <Network className="text-green-mid" size={28} />
            <h2 className="mt-5 text-2xl font-extrabold text-navy">{t.nonDiscriminationTitle}</h2>
            <p className="mt-4 text-sm leading-7 text-charcoal/65">{t.nonDiscrimination}</p>
          </article>
          <article className="rounded-lg bg-green-deep p-8 text-white sm:p-10">
            ...
            <a href={CONTACT_URL} className="button-light mt-6" aria-label={`Email ${CONTACT_EMAIL}`}>
              {CONTACT_EMAIL}
            </a>
          </article>
        </div>
        ...
        <div className="container-page flex flex-col gap-5 rounded-lg border border-green-deep/10 bg-paper p-6 shadow-soft md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-charcoal/60">
              {language === "en"
                ? "SEED connects citizens, experts, civic groups, companies, and supporters through transparent records and practical experiments."
