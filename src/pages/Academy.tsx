...
import { academyTranslations } from "../data/contentTranslations/staticPrograms";
...
export default function Academy() {
  ...
  const items = academyItems.map((item, index) => ko ? item : { ...item, ...academyTranslations[index] });
      ...
      <header className="border-b border-green-deep/15 bg-ivory">
        <div className="container-page grid gap-8 py-14 sm:py-20 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
          <div>
            ...
            <h1 className="editorial-title mt-4 text-5xl font-bold text-navy sm:text-6xl">{ko ? "씨드 아카데미" : "SEED Academy"}</h1>
          </div>
          <p className="max-w-2xl text-base leading-8 text-charcoal/65">
            {ko ? "시민이 사실을 읽고 권력을 이해하며 제안으로 이어갈 수 있도록, 실제 정치·사회 이슈를 시민의 언어로 배우는 과정입니다." : "SEED Academy helps citizens read facts, understand power and turn public questions into proposals by learning through real political and social issues."}
          </p>
        </div>
      </header>
