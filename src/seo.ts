...
const staticRoutes: SeoRoute[] = [
  ...
  { path: "/search", title: "통합검색 | 씨앗의 소리", description: "씨앗의 소리의 뉴스, 브리핑, 칼럼과 씨앗언어 콘텐츠를 한 번에 검색합니다.", type: "website", lastModified: latestDate },
];
...
const seedLanguageRoutes: SeoRoute[] = allSeedLanguageArticlesKo.map((article) => ({
  ...
  author: SITE_NAME,
  section: "씨앗언어",
  image: socialImageUrl("seed-language", article.slug, `${article.date}-${article.slug.startsWith("environment-") ? "3" : "2"}`),
  imageAlt: article.heroImage.alt,
}));
